...
function sayHello(call, callback) {
  ...
  callback(null, { message: `${new Date().toJSON()} - Hello <${call.request.name}>` });
}

function sayHelloAgain(call, callback) {
  callback(null, { message: `${new Date().toJSON()} - Hello again, <${call.request.name}>` });
}
...
function main() {
  ...
  server.addService(nsProto.Greeter.service,
